/* 
 * File:   dee_emulation.h
 * Author: LENOVO
 *
 * Created on July 24, 2025, 1:38 PM
 */

#ifndef DEE_EMULATION_H
#define	DEE_EMULATION_H

#include <stdio.h>
#include <string.h>
#include "definitions.h"

#define EEDATA_ADDRESS1                  (0x9D0F0000) 
#define EEDATA_ADDRESS2                  (0x9D1F0000) 

//#define DATA_EE_SIZE                    (2728)  // Total number of 32-bit data
#define DATA_EE_SIZE                    (1024)  // Total number of 32-bit data

#define NUM_PAGE1 3
#define NUM_PAGE2 3
#define NUM_DATA_EE_PAGES               (6)     // Total number of pages reserved for the operation

// Internal constants
#define ERASE_WRITE_CYCLE_MAX           (100000) // Maximum erase cycle per page
#define NUMBER_OF_INSTRUCTIONS_IN_PAGE  (4096) // number of 32-bit word instructions per page
//#define DATA_OFFSET                     (5456) // The point where data starts
#define DATA_OFFSET                     (2048) // The point where data starts
#define PIC32MZ_PAGE_SIZE               (NUMBER_OF_INSTRUCTIONS_IN_PAGE*4) // Total page size in bytes
...
extern "C" {
    ...
    typedef enum {
        PAGE_STATUS_VALID = 0, // page is valid
        PAGE_STATUS_EXPIRED = 2, // page expired
        PAGE_STATUS_CORRUPT = 6, // page is corrupt
        PAGE_STATUS_CHECKSUM_ERROR = 9, // page is checksum error
        PAGE_STATUS_ERASED = 11, // Page is erased (0xFFFFFFFF)
        PAGE_STATUS_INITIALIZED = 12, // Page is init (0x00000000)
        PAGE_STATUS_INACTIVE = 13 // Page contains valid data but is not active
    } PAGE_STATUS;

...
static const HMI_TAG_ENTRY _hmiTagTable[HMI_TAG_MAX_COUNT] = {
    ...
    { .startAddress = HMI_ADDR_ROW_NAME(10), .dataSize = 160},
    [HMI_TAG_PAGE3_ROW_UNIT] =
    { .startAddress = HMI_ADDR_ROW_UNIT(10), .dataSize = 160},
    [HMI_TAG_PAGE4_ROW_NAME] =
    { .startAddress = HMI_ADDR_ROW_NAME(15), .dataSize = 160},
    [HMI_TAG_PAGE4_ROW_UNIT] =
    { .startAddress = HMI_ADDR_ROW_UNIT(15), .dataSize = 160},

    [HMI_TAG_DEVICE_INFO] =
    { .startAddress = HMI_ADDR_DEVICE_INFO, .dataSize = 8},
};

static const char * __TAG__ = "HMI";
...
static uint16_t _buildFrameData(HMI_TAG_TYPE type, uint8_t maxSize, uint16_t startAddress, uint16_t *targetAddress) {
    ...
    switch (type) {
        ...
        {
            snprintf((char*) payload, maxSize, "%s", "v2.4.0");
            *targetAddress = startAddress;
            return true;
        }
        ...
        case HMI_TAG_PAGE1_ROW_NAME:
        {
            for (uint8_t i = 0; i < (MAX_HMI_PARA / 4 * 1); i++) {
                uint8_t idxHMI = gAppCfg.hmi.sensorIdx[i];
                SENSOR_ENTRY_CONFIG *sEntry = &gSensorCfg.entry[idxHMI - 1];

                if (idxHMI > 0 && idxHMI < MAX_SENSOR && sEntry->enable) {
                    uint8_t indexOfType = sEntry->indexOfType;
                    uint16_t offset;
                    uint8_t len;
                    char * str = NULL;
                    ...
                    else if (sEntry->type == SENSOR_ANALOG)
                        str = gAnalogCfg.entry[indexOfType].name;
                    else if (sEntry->type == SENSOR_INPUT_CAPTURE)
        ...
        case HMI_TAG_PAGE3_ROW_NAME:
        {
            for (uint8_t i = (MAX_HMI_PARA / 4 * 2); i < (MAX_HMI_PARA / 4 * 3); i++) {
                uint8_t idxHMI = gAppCfg.hmi.sensorIdx[i];
                SENSOR_ENTRY_CONFIG *sEntry = &gSensorCfg.entry[idxHMI - 1];
        ...
        case HMI_TAG_PAGE4_ROW_NAME:
        {
            for (uint8_t i = (MAX_HMI_PARA / 4 * 3); i < MAX_HMI_PARA; i++) {
                uint8_t idxHMI = gAppCfg.hmi.sensorIdx[i];
                SENSOR_ENTRY_CONFIG *sEntry = &gSensorCfg.entry[idxHMI - 1];
        ...
        case HMI_TAG_DEVICE_STATUS:
        {
            ...
            if (sdcardDt.status == SDCARD_STS_NOINSERT) snprintf(str, sizeof (str), "NOINSERT");
            else if (sdcardDt.status == SDCARD_STS_ERROR) snprintf(str, sizeof (str), "ERROR");
            else if (sdcardDt.status == SDCARD_STS_INSERTED) snprintf(str, sizeof (str), "INSERTED");
            else if (sdcardDt.status == SDCARD_STS_READY) snprintf(str, sizeof (str), "READY");
            else if (sdcardDt.status == SDCARD_STS_GOOD) snprintf(str, sizeof (str), "GOOD");
            ...
            if (ssGeneralDt.ftpStatus == FTP_STS_GOOD) snprintf(str, sizeof (str), "GOOD");
            else if (ssGeneralDt.ftpStatus == FTP_STS_ERROR) snprintf(str, sizeof (str), "ERROR");
            else snprintf(str, sizeof (str), "UNK");

            offset = (HMI_ADDR_FTP_STS - startAddress) * 2;
            len = strlen(str);
            len = (len <= ((HMI_ADDR_MQTT_STS - HMI_ADDR_FTP_STS) * 2)) ? len : ((HMI_ADDR_SIM_STS - HMI_ADDR_FTP_STS) * 2);
            memcpy((char *) &payload[offset], str, len);


            SIM_BASIC_INFO* simInfo = SIMBasic_GetInfo();
            if (simInfo->inserted) snprintf(str, sizeof (str), "INSERTED");
            else snprintf(str, sizeof (str), "NOINSERT");

            offset = (HMI_ADDR_SIM_STS - startAddress) * 2;
            len = strlen(str);
            len = (len <= ((HMI_ADDR_MQTT_STS - HMI_ADDR_SIM_STS) * 2)) ? len : ((HMI_ADDR_MQTT_STS - HMI_ADDR_SIM_STS) * 2);
            memcpy((char *) &payload[offset], str, len);


            snprintf(str, sizeof (str), "READY");
            offset = (HMI_ADDR_MQTT_STS - startAddress) * 2;
            len = strlen(str);
            len = (len <= ((endAddress - HMI_ADDR_MQTT_STS) * 2)) ? len : ((endAddress - HMI_ADDR_MQTT_STS) * 2);
            memcpy((char *) &payload[offset], str, len);
...
void HMIDwin_Tasks() {
    ...
    if (!bootFirst && (currentTick - bootTick > tickPerSecond * 5)) {
        ...
        _enqueue(&_hmiTagQueue, &tagType);
        tagType = HMI_TAG_PAGE3_ROW_NAME;
        _enqueue(&_hmiTagQueue, &tagType);
        tagType = HMI_TAG_PAGE3_ROW_UNIT;
        _enqueue(&_hmiTagQueue, &tagType);
        tagType = HMI_TAG_PAGE4_ROW_NAME;
        _enqueue(&_hmiTagQueue, &tagType);
        tagType = HMI_TAG_PAGE4_ROW_UNIT;
        _enqueue(&_hmiTagQueue, &tagType);
    }
    ...
    if (_hmiBootReady) {
        ...
        if (currentTick - cleanTick > tickPerSecond * 5) {
            ...
            for (uint8_t i = 0; i < MAX_HMI_PARA; i++) {
                uint8_t idxHMI = gAppCfg.hmi.sensorIdx[i];
                if (idxHMI < MAX_SENSOR && !gSensorCfg.entry[idxHMI].enable) {
...
bool HMIDwin_TriggerSendStatus(uint8_t idxRow, uint8_t status) {
    ...
    if (status == 0)
        ...
        _statusPending.entry[idxRow].buffer[0] = '3';
